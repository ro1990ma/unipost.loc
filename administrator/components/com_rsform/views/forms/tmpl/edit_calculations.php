<?php
/**
* @version 1.4.0
* @package RSform!Pro 1.4.0
* @copyright (C) 2007-2013 www.rsjoomla.com
* @license GPL, http://www.gnu.org/copyleft/gpl.html
*/

defined('_JEXEC') or die('Restricted access'); ?>

<div id="rsform_layout_msg" class="rsform_error">
	<b><?php echo JText::_('RSFP_CALCULATION_INFO'); ?></b>
</div>
<br />
<table class="admintable">
	<tr>
		<td>
			<select name="rsfp_total_add" id="rsfp_total_add" size="1" style="margin-bottom:0px;">
				<?php echo JHtml::_('select.options',$this->totalFields); ?>
			</select>
		</td>
		<td> = </td>
		<td>
			<input type="text" name="rsfp_expression" id="rsfp_expression" size="100" class="rs_inp rs_80" value="" onclick="toggleDropdown(this,0,1);" onkeydown="closeAllDropdowns();" />
			<a href="javascript:void(0)" onclick="rsfp_add_calculation(<?php echo $this->form->FormId; ?>);">
				<img src="<?php echo JURI::root(); ?>administrator/components/com_rsform/assets/images/add.png" alt="" />
			</a>
		</td>
	</tr>
	<tr><td colspan="3">&nbsp;</td></tr>
	<tbody id="rsfp_calculations">
		<?php if (!empty($this->calculations)) { ?>
		<?php foreach ($this->calculations as $row) { ?>
		<tr id="calculationRow<?php echo $row->id; ?>">
			<td>
				<select name="calculations[<?php echo $row->id; ?>][total]" size="1" style="margin-bottom:0px;">
					<?php echo JHtml::_('select.options',$this->totalFields,'value','text',$row->total); ?>
				</select>
			</td>
			<td> = </td>
			<td>
				<input type="text" name="calculations[<?php echo $row->id; ?>][expression]" id="calculations<?php echo $row->id; ?>expression" size="100" class="rs_inp rs_80" onclick="toggleDropdown(this,0,1);" onkeydown="closeAllDropdowns();" value="<?php echo $this->escape($row->expression); ?>" />
				<a href="javascript:void(0)" onclick="rsfp_remove_calculation(<?php echo $row->id; ?>);">
					<img src="<?php echo JURI::root(); ?>administrator/components/com_rsform/assets/images/close.png" alt="" />
				</a>
				<input type="hidden" name="calcid[]" value="<?php echo $row->id; ?>" />
				<input type="hidden" name="calorder[]" value="<?php echo $row->ordering; ?>" />
			</td>
		</tr>
		<?php } ?>
		<?php } ?>
	</tbody>
</table>