...
function servis_tabs(event) {

        console.log("click");
    }

function set_active2(art){
BTLJ('p#inf').removeClass('active');
BTLJ('#'+art).parent().addClass('active');
var top = BTLJ('#'+art).parent().offset().top-300;
BTLJ('body,html').delay(500);
BTLJ('body,html').animate({'scrollTop': top}, 300);
};  

BTLJ(document).ready(function() {
    
  BTLJ('.servis_tabs li a').click(function(event){
        event.preventDefault();
        var id  = BTLJ(this).attr('href'),
            top = BTLJ(id).offset().top;
        BTLJ('body,html').animate({scrollTop: top}, 1500);
});
...
//console.log("TEST2"+document.location.search);
addMest();

$("div#page_tabs ul li").each(function(index,value){
$(this).addClass('ui-state-active');
});

$("div#page_tabs ul li a").click(function(){
//    console.log("click");
...
//$('#vtemailForm').validate();
});

function replacer_Name() {
var str = BTLJ("input[name=vtem_name]").val();
str=str.replace(/[^a-zA-ZА-Яа-я\s]/g,"");
console.log(str);
BTLJ("input[name=vtem_name]").val(str);
}

function replacer_subject() {
var str = BTLJ("input[name=vtem_subject]").val();
str=str.replace(/[^a-zA-ZА-Яа-я\s]/g,"");
console.log(str);
BTLJ("input[name=vtem_subject]").val(str);
}

function replacer_gorod() {
var str = BTLJ("input[name=vtem_gorod]").val();
str=str.replace(/[^a-zA-ZА-Яа-я\s]/g,"");
console.log(str);
BTLJ("input[name=vtem_gorod]").val(str);
}

function replacer_email() {
var str = BTLJ("input[name=vtem_email]").val();
str=str.replace(/[^\.\-a-zA-ZА-Яа-я\s]/g,"");
console.log(str);
BTLJ("input[name=vtem_email]").val(str);
}

function replacer_phone() {
var str = BTLJ("input[name=vtem_phone]").val();
str=str.replace(/[^0-9\+\s]/g,"");
console.log(str);
BTLJ("input[name=vtem_phone]").val(str);
}

function addMest(){
var mest = BTLJ("#calc_size_lenght option:selected").text();
mest=Number(mest);
var GABARITI=BTLJ("#GABARITI").val();
var GABARITI_SM=BTLJ("#GABARITI_SM").val();
var LENGHT=BTLJ("#LENGHT").val();
var WIDTH=BTLJ("#WIDTH").val();
var HEIGHT=BTLJ("#HEIGHT").val();
BTLJ(".gab_add").remove();
for(var i=2; i<=mest; i++){
//    console.log(i);

BTLJ("#gabariti_"+(i-1)).after("<tr id='gabariti_"+i+"' class='gab_add'>\n\
<td><p>"+GABARITI+" "+i+" "+GABARITI_SM+"</p></td>\n\
<td colspan='3'><input class='short' type='text' onchange='setCalcHandle()' id='len_"+i+"' placeholder='"+LENGHT+"' name='calc_size_lenght_gab_"+i+"' /> x <input class='short' id='wid_"+i+"' type='text' onchange='setCalcHandle()' placeholder='"+WIDTH+"' name='calc_size_width_gab_"+i+"' /> x <input class='short' id='hei_"+i+"' type='text' onchange='setCalcHandle()' placeholder='"+HEIGHT+"' name='calc_size_height_gab_"+i+"' /></td></tr>");
...
var totalweight=0;
var sel_mest=Number(BTLJ("#calc_size_lenght").val());

        BTLJ("#weight2_error").css("display","none");
        BTLJ("#do_calculate").attr("disabled", "true");

var volweight = funcCalcVolumeWeight(BTLJ('#len').attr('value'), BTLJ('#wid').attr('value'), BTLJ('#hei').attr('value'));
//BTLJ('#req_to_contact_pers_volume').val()
totalweight=Number(volweight);
      
if(sel_mest>1){
    
var z = new Array();
for(var i=2; i<=sel_mest; i++){

console.log(funcCalcVolumeWeight(BTLJ('#len_'+i).attr('value'), BTLJ('#wid_'+i).attr('value'), BTLJ('#hei_'+i).attr('value')));
z[i] = funcCalcVolumeWeight(BTLJ('#len_'+i).attr('value'), BTLJ('#wid_'+i).attr('value'), BTLJ('#hei_'+i).attr('value'));
//console.log(" "+BTLJ('#len_'+i).attr('value')+" "+BTLJ('#wid_'+i).attr('value')+" "+BTLJ('#hei_'+i).attr('value'));
      if (z[i] > 60 || volweight > 60)
      {
           BTLJ("#weight2_error").css("display","block");
           BTLJ("#do_calculate").attr("disabled", "true");
      }
//      else 
//      {
//        BTLJ("#weight2_error").css("display","none");
//        BTLJ("#do_calculate").removeAttr('disabled');
//      }
totalweight=(totalweight+Number(z[i]));
}}else{
//console.log(" "+BTLJ('#len_'+i).attr('value')+" "+BTLJ('#wid_'+i).attr('value')+" "+BTLJ('#hei_'+i).attr('value'));
      if (totalweight > 60)
      {
           ...
           BTLJ("#do_calculate").attr("disabled", "true");
            ...
            BTLJ('#req_to_contact_pers_volume').attr('value', totalweight.toFixed(3));
