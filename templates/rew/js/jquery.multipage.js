	...
	jQuery.fn.multipage = function(options) { 
		...
		$(id +' fieldset').hide();
			...
			if ($(id+' input[type="submit"]').val()!='') { 
				settings.submitLabel = settings.submitLabel;
			}
			...
			if (settings.generateNavigation) { 
				if (settings.navigationFunction) { 
					...
					// insert navigation
                        ...
                        $('<div class="multipage_nav" id="'+id_name+'_nav"><span class="multipage_state"></span><a href="#"  class="multipage_next" onclick="return $(\''+id+'\').nextpage();">Next</a><a href="#" class="multipage_back" onclick="return  $(\''+id+'\').prevpage();">Back</a><div class="clearer"></div></div>').insertAfter(this);
				}
			}				
